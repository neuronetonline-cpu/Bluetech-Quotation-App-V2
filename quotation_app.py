...
class App:
    ...
    def save_quote(self):
        ...
        c.execute("""INSERT INTO quotations(qno,customer,phone,date,profit,warranty90,warranty180,weight,created_at)
                  ...
                  (self.qno.get(), self.customer.get(), self.phone.get(), self.qdate.get(), self.num(self.profit.get()),
                   self.num(self.final90.get()), self.num(self.final180.get()), self.num(self.weight.get()), datetime.now().isoformat()))
    ...
    def save_pdf(self):
        ...
        header_left = [Paragraph("<b><font color='#1478D4'>BLUE</font>TECH</b>", ParagraphStyle("logo", parent=title, fontSize=21, leading=21)),
                       Paragraph("<b>COMPUTERS</b>", ParagraphStyle("comp", parent=subtitle, fontSize=10, leading=11, textColor=colors.HexColor(DARK_BLUE))),
                       Paragraph("Computer Sales | Repairs | Upgrades", subtitle)]
        contact = Paragraph("<b>077 988 9693</b><br/><b>071 321 9693</b><br/>No. 123, Highlevel Road, Maharagama<br/>bluetechcomputers.lk@gmail.com", info_style)
        ...
        w90 = Table([[Paragraph("WITH <font color='#075EAA'>90 DAYS</font><br/>HARDWARE WARRANTY", warranty_style),
                      Paragraph(money(p90), price_style)]], colWidths=[91 * mm, 89 * mm])
        w90.setStyle(TableStyle([
            ("BACKGROUND", (0, 0), (-1, -1), colors.HexColor(LIGHT_BLUE)),
            ("BOX", (0, 0), (-1, -1), 0.7, colors.HexColor("#B9DBF8")),
            ("BACKGROUND", (1, 0), (1, 0), colors.HexColor(BLUE)),
            ("VALIGN", (0, 0), (-1, -1), "MIDDLE"),
            ("LEFTPADDING", (0, 0), (-1, -1), 8), ("RIGHTPADDING", (0, 0), (-1, -1), 8),
            ("TOPPADDING", (0, 0), (-1, -1), 8), ("BOTTOMPADDING", (0, 0), (-1, -1), 8),
        ]))
        w180 = Table([[Paragraph("WITH <font color='#159447'>180 DAYS</font><br/>HARDWARE WARRANTY", warranty_style),
                       Paragraph(money(p180), price_style)]], colWidths=[91 * mm, 89 * mm])
        ...
        story.append(Table([[w90, w180]], colWidths=[91 * mm, 89 * mm], style=TableStyle([
            ("LEFTPADDING", (0, 0), (-1, -1), 0), ("RIGHTPADDING", (0, 0), (-1, -1), 0),
            ("TOPPADDING", (0, 0), (-1, -1), 0), ("BOTTOMPADDING", (0, 0), (-1, -1), 0),
        ])))
        story.append(Spacer(1, 7))

        terms = Paragraph("<b>Terms & Conditions</b><br/>• Prices are valid for 7 days from the quotation date.<br/>• Warranty covers hardware faults only; physical damage, burns and liquid damage are not covered.<br/>• Stock availability may change without prior notice.<br/>• For more details, contact us by phone or WhatsApp.", small)
    ...
    def history(self):
        ...
        c = db()
        for row in c.execute("SELECT qno,customer,phone,date,profit,warranty90,warranty180 FROM quotations ORDER BY id DESC"):
